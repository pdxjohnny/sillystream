...
def master(args):
    ...
    __server_process__ = sillystring.sillystring()
...
def start(args):
    global __server_process__
    del args["recv"]
    __server_process__ = sillystring.server()
    if "username" in args and "password" in args:
        global AUTH_USER
        global AUTH_PASS
        AUTH_USER = args["username"]
...
def connect(args):
    ...
    del args["recv"]
    __client_conn__ = sillystring.client()
...
def arg_setup():
    global ARG_PARSER
    ARG_PARSER = argparse.ArgumentParser(description=sillystring.__description__)
    ...
    ARG_PARSER.add_argument("--port", type=int, \
        help="Port to host or connect to sillystring server")
    ...
    ARG_PARSER.add_argument("--username", "-u", type=unicode, \
        help="Username to connect to sillystring server")
    ARG_PARSER.add_argument("--password", "-p", type=unicode, \
        help="Password to connect to sillystring server")
    ...
    ARG_PARSER.add_argument("--version", "-v", action="version", \
        version=u"sillystring " + unicode(sillystring.__version__) )
